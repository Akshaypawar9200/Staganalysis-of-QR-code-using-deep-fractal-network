<%@page import="com.documentsharing.bean.Follower"%>
...
<body>
	...
	<div id="wrapper">
        ...
        <%} %>
    		...
    		<tbody>
    		<%
    			ArrayList<Follower> listFollower = (ArrayList<Follower>)session.getAttribute("listFollower");
    			if(listFollower!=null){
    				for(int i=0;i<listFollower.size();i++){
    		%>
    		<tr>
    			<th><img alt="Profile Pic" class="img-square" src="profilepic.jsp?id=<%=listFollower.get(i).getReceiver().getUserId() %>"  width="50px" height="50px"/></th>
        		<th><%=listFollower.get(i).getReceiver().getFname()+" "+listFollower.get(i).getReceiver().getLname() %></th>
       			<th><%=listFollower.get(i).getReceiver().getEmail() %></th>
        		<th><%=listFollower.get(i).getReceiver().getDob() %></th>
       			<th><%=listFollower.get(i).getReceiver().getContact() %></th>
       			<th><%=listFollower.get(i).getReceiver().getAddress() %></th>
       			<th><%=listFollower.get(i).getStatus() %></th>
       			<th><%=listFollower.get(i).getRequestDate() %></th>
       			<th><a style="color:blue" href="FriendStatus1Controller?userId=<%= userId%>&followerId=<%=listFollower.get(i).getReceiver().getUserId() %>&status=<%=listFollower.get(i).getStatus()%>">Accepted</a>
       				&nbsp;<a href="FriendStatus2Controller?userId=<%= userId%>&followerId=<%=listFollower.get(i).getReceiver().getUserId() %>&status=<%=listFollower.get(i).getStatus()%>">Rejected</a></th>
