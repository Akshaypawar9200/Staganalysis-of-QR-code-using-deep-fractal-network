				...
				<div class="row">
					<div class="col-lg-12">
						<ul class="breadcrumb">
							...
							<li class="active">Post Message</li>
						</ul>
					</div>
				</div>
			</div>
        ...
        <%} %>
							...
							<div class="widget">
								...
								<ul class="cat">
									...
									<li class="active"><i class="fa fa-angle-right"></i><a href="UserFriendListController?userId=<%= userId %>">Post Message</a></li>
					...
					<div class="col-lg-6">						
						...
						<form action="UserPostController" method="post"
								...
								<div align="left">									
										...
										<div id="publicMsg">
											<div class="form-group">
												<input type="hidden" name="userId"
													value="<%=session.getAttribute("userId")%>"> <label>Post
													your public message</label>
												<textarea class="form-control"
													placeholder="Type here message" name="pubmessage" rows="3"></textarea>
											</div>
										...
										<div id="privateMsg" style="display:none">
											<div class="form-group">
												<input type="hidden" name="userId"
													value="<%=session.getAttribute("userId")%>"> <label>Post
													your private message</label>
												<textarea class="form-control"
													placeholder="Type here message" name="privmessage" rows="3"></textarea>
											</div>
											
											<div class="form-group">
											<label>Enter the number of parts to create:</label>
											<input type="text" class="form-control" name="partnum" id="partnum" placeholder="Number of parts" value="" pattern="[0-9]+" title="Number must be digits only">
											<p>(Integer at least 1.)</p>
											</div>
											<div class="form-group">
											<label>Enter the number of parts required to reconstruct the secret:</label>
											<input type="text" class="form-control" name="reconstructnum" id="reconstructnum" placeholder="Number of reconstruct secret" value="" pattern="[0-9]+" title="Number must be digits only">
											<p>(Integer at least 1, no more than number of total parts.)</p>
											</div>
											<%
    			ArrayList<User> friendList = (ArrayList<User>)session.getAttribute("friendList");
    			if(friendList!=null){
    				for(int i=0;i<friendList.size();i++){
    					String friendname = friendList.get(i).getFname()+" "+friendList.get(i).getLname();
    		%>    		
                        <div><input type="checkbox" id="<%=friendList.get(i).getUserId() %>" name="friendname" value="<%=friendList.get(i).getUserId() %>">&nbsp;<%=friendname %></div>                          			
    		<%
    				}
    			}
    		%>
