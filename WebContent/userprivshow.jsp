...
<body>
	...
	<div id="wrapper">
        ...
        <%} %>
					...
					<div class="col-lg-6">						
						<hr class="colorgraph">
						<h3>Decoded QR code Result</h3>
						<form action="UserViewMsgController?userId=<%=userId %>" method="post">
								<div align="left">
								<%
								ArrayList<String> msgList=(ArrayList<String>) request.getAttribute("SecretMsgList");
								String message = (String) request.getAttribute("PrivMsg");
								%>
								<div>
								<textarea rows="5" cols="50"><%for(int i=0;i<msgList.size();i++){ %>
								<%=msgList.get(i)+"\n" %><%} %>
								</textarea>
								</div>
								<div>
								<textarea rows="5" cols="50"><%=message %></textarea>
										...
										<p class="left col-md-5">
											<button type="submit" class="btn btn-theme">Back</button>
										</p>
